...
# # Изменения: Переход на SQLite с aiosqlite, сохранена структура БД
...
#
# # Настройка асинхронного и синхронного движков для SQLite
# async_engine = create_async_engine(settings.database_url_async, echo=False)
...
#         setting = result.scalars().first()
#         return int(setting.value) if setting else None
# Изменения: Добавлено создание директории для файла БД перед инициализацией
from sqlalchemy import create_engine, select, func
from sqlalchemy.ext.asyncio import create_async_engine, async_sessionmaker, AsyncSession
from sqlalchemy.orm import sessionmaker
from shared.models import Base, User, Site, SystemSettings
...
# Создание директории для файла БД, если она не существует
...
async def init_db():
    ensure_db_directory()  # Гарантируем наличие директории
    async with async_engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
