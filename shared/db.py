...
# # from shared.logger_setup import logger
# #
# # # Настройка асинхронного движка и сессий
...
# #
# # # Настройка синхронного движка и сессий (для Celery)
...
# #         await conn.run_sync(Site.metadata.create_all)
...
# #     async with AsyncSessionFactory() as session:
# #         try:
# #             # Запрос с LEFT JOIN и COUNT для подсчета сайтов
# #             stmt = (
# #                 select(
# #                     User.id,
# #                     User.telegram_id,
...
# #
# # async def get_user_sites_admin(user_id: int) -> List[Site]:
# #     """Получает все сайты пользователя."""
# #     async with AsyncSessionFactory() as session:
# #         result = await session.execute(select(Site).where(Site.user_id == user_id))
# #         return result.scalars().all()
# #
# #
# # async def delete_site_admin(site_id: int) -> int | None:
# #     """Удаляет сайт и возвращает ID пользователя."""
# #     async with AsyncSessionFactory() as session:
# #         try:
